...
//		$this->data_color = array('blue','green','yellow','red','orange');
		$this->data_color = array_rand($this->rgb_array , count($this->rgb_array));
...
//ajjjo
	...
	function DrawPieChart() {
		...
		while (list(, $val) = each($sumarr)) {
			...
			$label_txt = $val." (".number_format(($val / $total * 100), $this->y_precision, ".", ",") . "%)";
			$val = 360 * ($val / $total);
...
//echo "$row[0],$row[1],$row[2],$row[3],$row[4],$row[5],$row[6],$row[7],$row[8],$row[9],$row[10],$row[11],$row[12], $barcol<br>";
	...
	//Lower Right
		...
		$box_end_y = $box_start_y + $line_spacing*$this->small_font_height*(count($this->legend)+1) + 2*$vert_margin; 
